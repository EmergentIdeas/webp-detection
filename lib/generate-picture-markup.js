 ...
 * @param {object} [options.imgClass] Class attribute for the image element
...
function generatePictureMarkup(url, {width, height, format, alt, 
	displayWidth, displayHeight, pictureStyle, imgStyle, imgClass, pictureClass, cdnPrefix = ''} = {}) {
...
`<picture class="${pictureClass}" ${pictureStyleAttr}>
	...
	<img src="${cdnPrefix}${baseUrl}.${ext}" alt="${alt}" width="${displayWidth}" height="${displayHeight}" ${imgStyleAttr} ${imgClassAttr}>
</picture>
`
	}
	else {
